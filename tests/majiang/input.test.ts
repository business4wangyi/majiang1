...
import { PlayerAction } from '../../src/majiang/core/rule-types';
...
import {
  ...
  InputState,
  InputOptions,
  setReadlineInterfaceFactory,
  resetReadlineInterfaceFactory
} from '../../src/majiang/ui/input';

// 导入CountdownManager以便在测试中引用
import { CountdownManager } from '../../src/majiang/ui/countdown-manager';
...
describe('Input Module - 基本函数', () => {
  ...
  beforeEach(() => {
    ...
    setReadlineInterfaceFactory(() => mockReadline);
  });
  ...
  describe('askQuestion', () => {
    ...
    it('应正确处理用户输入', async () => {
      ...
      mockReadline.question.callsFake((_, callback) => callback(userInput));
    ...
    it('应正确处理空输入', async () => {
      ...
      mockReadline.question.callsFake((_, callback) => callback(''));
...
describe('InputState', () => {
  ...
  let originalWaitingDescriptor: PropertyDescriptor | undefined;
  ...
  beforeEach(() => {
    ...
    originalWaitingDescriptor = Object.getOwnPropertyDescriptor(CountdownManager, 'isWaitingForUserInput');
  ...
  afterEach(() => {
    ...
    if (originalWaitingDescriptor) {
      Object.defineProperty(CountdownManager, 'isWaitingForUserInput', originalWaitingDescriptor);
    }

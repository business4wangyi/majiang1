import { expect } from 'chai';
import { SmallFourWindsDetector } from '../../../src/majiang/core/win-conditions/win-conditions_small-four-winds';
import { Tile, TileType, FengValue } from '../../../src/majiang/core/tile';
import { TileSet } from '../../../src/majiang/core/rule-types';

// 辅助函数
function createPung(type: TileType, value: number, startId = 1): TileSet {
  return {
    type: 'PENG',
    tiles: [
      new Tile(type, value, startId),
      new Tile(type, value, startId + 1),
      new Tile(type, value, startId + 2)
    ]
  };
}

function createKong(type: TileType, value: number, startId = 1): TileSet {
  return {
    type: 'GANG',
    tiles: [
      new Tile(type, value, startId),
      new Tile(type, value, startId + 1),
      new Tile(type, value, startId + 2),
      new Tile(type, value, startId + 3)
    ]
  };
}

function createChow(type: TileType, startValue: number, startId = 1): TileSet {
  return {
    type: 'CHI',
    tiles: [
      new Tile(type, startValue, startId),
      new Tile(type, startValue + 1, startId + 1),
      new Tile(type, startValue + 2, startId + 2)
    ]
  };
}

function createPair(type: TileType, value: number, startId = 1): Tile[] {
  return [
    new Tile(type, value, startId),
    new Tile(type, value, startId + 1)
  ];
}

function createHandTilesForChow(type: TileType, startValue: number, startId = 1): Tile[] {
  return [
    new Tile(type, startValue, startId),
    new Tile(type, startValue + 1, startId + 1),
    new Tile(type, startValue + 2, startId + 2)
  ];
}

function createHandTilesForPung(type: TileType, value: number, startId = 1): Tile[] {
  return [
    new Tile(type, value, startId),
    new Tile(type, value, startId + 1),
    new Tile(type, value, startId + 2)
  ];
}

describe('SmallFourWindsDetector', () => {
  const detector = new SmallFourWindsDetector();
  // 枚举所有x副吃、y副碰、z副杠，x+y+z<=4
  const cases: [number, number, number][] = [];
  for (let x = 0; x <= 4; x++) {
    for (let y = 0; y <= 4; y++) {
      for (let z = 0; z <= 4; z++) {
        if (x + y + z <= 4) {
          cases.push([x, y, z]);
        }
      }
    }
  }
  // 只保留35种
  expect(cases.length).to.equal(35);

  let caseIdx = 1;
  cases.forEach(([chiCount, pengCount, kongCount], idx) => {
    const title = `吃${chiCount} 碰${pengCount} 杠${kongCount}`;
    
    // 能胡牌测试：保证有3种风牌的刻子/杠和1种风牌的对子
    it(`Case ${caseIdx++}: 能胡牌: ${title}`, () => {
      // 明牌组合
      const revealedSets: TileSet[] = [];
      let id = 1;
      
      // 先添加风牌组合，优先添加到明牌中
      let windPungCount = 0;
      
      // 添加风牌刻子和杠到明牌中
      const maxRevealedWinds = Math.min(3, pengCount + kongCount);
      for (let i = 0; i < maxRevealedWinds; i++) {
        if (windPungCount < 3) {
          if (i < kongCount) {
            revealedSets.push(createKong(TileType.FENG, i + 1, id));
          } else {
            revealedSets.push(createPung(TileType.FENG, i + 1, id));
          }
          windPungCount++;
          id += i < kongCount ? 4 : 3;
        }
      }
      
      // 添加剩余的非风牌明牌
      const remainingRevealedSets = (pengCount + kongCount) - maxRevealedWinds;
      for (let i = 0; i < remainingRevealedSets; i++) {
        if (i < (kongCount - maxRevealedWinds)) {
          revealedSets.push(createKong(TileType.TONG, i + 1, id));
          id += 4;
        } else {
          revealedSets.push(createPung(TileType.TIAO, i + 1, id));
          id += 3;
        }
      }
      
      // 添加吃的明牌
      for (let i = 0; i < chiCount; i++) {
        revealedSets.push(createChow(TileType.WAN, 1 + i, id));
        id += 3;
      }
      
      // 计算手牌中需要的风牌刻子数量
      const neededWindPungs = 3 - windPungCount;
      
      // 生成手牌
      let handTiles: Tile[] = [];
      
      // 添加风牌刻子到手牌
      for (let i = 0; i < neededWindPungs; i++) {
        // 确保风牌值在1-4之间
        const windValue = (maxRevealedWinds + i) % 4 + 1;
        handTiles = handTiles.concat(createHandTilesForPung(TileType.FENG, windValue, id));
        id += 3;
      }
      
      // 添加风牌对子
      // 找一个没有使用过的风牌值（1-4之间）
      let pairWindValue = 1;
      while (pairWindValue <= 4) {
        let used = false;
        
        // 检查明牌中是否已使用此风牌值
        for (const set of revealedSets) {
          if (set.tiles.length > 0 && 
              set.tiles[0].type === TileType.FENG && 
              set.tiles[0].value === pairWindValue) {
            used = true;
            break;
          }
        }
        
        // 检查手牌中刻子是否已使用此风牌值
        if (!used) {
          for (let i = 0; i < neededWindPungs; i++) {
            const windValue = (maxRevealedWinds + i) % 4 + 1;
            if (windValue === pairWindValue) {
              used = true;
              break;
            }
          }
        }
        
        if (!used) break;
        pairWindValue++;
      }
      
      handTiles = handTiles.concat(createPair(TileType.FENG, pairWindValue, id));
      id += 2;
      
      // 计算还需要多少组
      const remainingSets = 4 - (chiCount + pengCount + kongCount + neededWindPungs);
      
      // 添加其他组合到手牌
      for (let i = 0; i < remainingSets; i++) {
        handTiles = handTiles.concat(createHandTilesForChow(TileType.WAN, 5 + i, id));
        id += 3;
      }
      
      expect(detector.detect(handTiles, revealedSets)).to.equal(true);
    });
    
    // 不能胡牌测试：故意破坏小四喜的特性（如只有2种风牌刻子和2种风牌对子）
    it(`Case ${caseIdx++}: 不能胡牌: ${title}`, () => {
      // 明牌组合
      const revealedSets: TileSet[] = [];
      let id = 1;
      
      // 添加明牌
      let windPungCount = 0;
      
      // 添加刻子和杠到明牌中
      const maxRevealedWinds = Math.min(2, pengCount + kongCount); // 故意只放2种风牌刻子
      for (let i = 0; i < maxRevealedWinds; i++) {
        if (windPungCount < 2) {
          if (i < kongCount) {
            revealedSets.push(createKong(TileType.FENG, i + 1, id));
          } else {
            revealedSets.push(createPung(TileType.FENG, i + 1, id));
          }
          windPungCount++;
          id += i < kongCount ? 4 : 3;
        }
      }
      
      // 添加剩余的非风牌明牌
      const remainingRevealedSets = (pengCount + kongCount) - maxRevealedWinds;
      for (let i = 0; i < remainingRevealedSets; i++) {
        if (i < (kongCount - maxRevealedWinds)) {
          revealedSets.push(createKong(TileType.TONG, i + 1, id));
          id += 4;
        } else {
          revealedSets.push(createPung(TileType.TIAO, i + 1, id));
          id += 3;
        }
      }
      
      // 添加吃的明牌
      for (let i = 0; i < chiCount; i++) {
        revealedSets.push(createChow(TileType.WAN, 1 + i, id));
        id += 3;
      }
      
      // 生成手牌
      let handTiles: Tile[] = [];
      
      // 只在手牌中添加一种风牌刻子
      if (windPungCount < 2) {
        handTiles = handTiles.concat(createHandTilesForPung(TileType.FENG, 3, id));
        id += 3;
        windPungCount++;
      }
      
      // 添加两对风牌对子，而不是一对 - 这会破坏小四喜
      handTiles = handTiles.concat(createPair(TileType.FENG, 3, id));
      id += 2;
      handTiles = handTiles.concat(createPair(TileType.FENG, 4, id));
      id += 2;
      
      // 计算还需要多少组
      const remainingSets = 4 - (chiCount + pengCount + kongCount + (windPungCount < 2 ? 1 : 0));
      
      // 添加其他组合到手牌
      for (let i = 0; i < remainingSets; i++) {
        handTiles = handTiles.concat(createHandTilesForChow(TileType.WAN, 5 + i, id));
        id += 3;
      }
      
      expect(detector.detect(handTiles, revealedSets)).to.equal(false);
    });
  });
}); 
...
import { PengPengHuDetector } from '../../../src/majiang/core/win-conditions/win-conditions_peng-peng-hu';
...
describe('PengPengHuDetector', () => {
  const detector = new PengPengHuDetector();
  ...
  cases.forEach(([chiCount, pengCount, kongCount], idx) => {
    ...
    it(`Case ${caseIdx++}: 能胡牌: ${title}`, () => {
      ...
      for (let i = 0; i < setsNeeded; i++) {
        handTiles = handTiles.concat(createHandTilesForPung(TileType.WAN, 5 + i, id));
        id += 3;
      }
      handTiles = handTiles.concat(createPair(TileType.TIAO, 9, id));
      // 只要没有吃就能胡
      const canHu = chiCount === 0;
      expect(detector.detect(handTiles, revealedSets)).to.equal(canHu);
    });

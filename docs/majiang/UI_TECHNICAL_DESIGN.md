...
- Web 对局诊断日志与事件回放能力
  - 当前发布版 Web session 默认关闭底层麻将 console/file 日志，以保持页面 smoke 和服务输出干净
  - 仅凭 UI 结果页无法充分判断胡牌触发、响应动作、AI 推进、结算分数是否真实正确
  - 后续验收辅助能力应至少记录：sessionId、回合序号、当前玩家、摸牌/出牌、吃碰杠胡/过、胡牌触发来源、自摸/荣和、点炮者、胡牌牌型、结算前后分数、最终赢家
  - 诊断日志应由显式开关控制，例如 `MAJIANG_WEB_DIAGNOSTIC_LOG=1`，默认不影响正式发布运行输出
  - 验收时应能导出或查看单局事件日志，用于复核 UI 展示与底层规则推进是否一致
...
### 17.11 后续验收辅助能力：Web 对局诊断日志

本次发布版为了保持 Web smoke 与服务输出干净，`src/majiang/application/game-session.ts` 默认关闭底层麻将 console/file 日志。因此，如果只通过页面观察，很难在事后确认某一局的真实运行链路，例如：
...
- 点炮者是谁
- 结算前后各家的分数变化是否正确
- UI 展示的赢家与底层 `PlayerState.WON` 是否一致

后续进入验收增强阶段时，应补充 Web 对局诊断日志能力，作为 UI 验收的辅助工具，而不是依赖人工肉眼观察页面结果。建议最小方案如下：

- 增加显式诊断开关，默认关闭，不污染正式运行输出
- 每个 Web session 维护一份结构化事件日志
- 每次状态变化记录回合序号、玩家、动作、牌、响应动作、胡牌判定、分数变化
- 对局结束时记录最终赢家、胡牌类型、结算摘要和四家最终分数
- 提供只读调试接口或本地日志导出能力，便于验收人员复盘单局问题
- smoke 或专项脚本覆盖“结束态有赢家时，诊断日志、result 赢家、分数摘要三者一致”

该能力不属于当前 MVP 正式发布阻塞项，但应作为后续 UI 验收和问题定位的优先补强项。尤其当页面出现“未观察到胡牌但结果显示获胜”“赢家存在但分数全为 0”等现象时，必须通过诊断日志复核底层真实事件，而不能只根据 UI 结果页下结论。

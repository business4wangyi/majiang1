...
function buildReport(results, previous) {
  ...
  const lines = [
    ...
    '### 1. 结果来源',
    '',
    `- 回归报告文件：\`${path.relative(rootDir, reportPath)}\``,
    `- 动态跳过基线文件：\`${path.relative(rootDir, memoryPath)}\``,
    '- 基线文件用途：GitHub Actions 每日回归动态跳过感知（JSON）。',
    '- 说明：该 JSON 不是 Codex triage 的 Markdown P0 记忆文件。',
    '',
    '### 2. 回归结论',
    '',
    conclusion,
    ...
    '',
    '- P0: updated',
    `- P0 path: ${path.relative(rootDir, memoryPath)}`,
    `- updatedAt: ${generatedAt}`,
    '- writeMode: overwrite-json',
    '- reason: 写入 GitHub Actions 动态跳过基线文件（非 Codex triage Markdown P0 记忆）。',
    '- P1: 未使用（默认不写入、不关注）',
    '',
  ];

...
exports.cleanupGameLoop = cleanupGameLoop;
...
async function gameLoop(game) {
    ...
    gameLoopInterval = setInterval(async () => {
        ...
        try {
            ...
            // 检测游戏状态循环
            const { isLoop, repeatedState } = (0, game_health_1.detectStateLoop)(lastGameStates, currentGameState, 20);
            if (isLoop) {
                (0, logger_1.warnLog)(`检测到游戏状态可能循环: ${repeatedState}`);
                display_manager_1.displayManager.printWarning(`检测到游戏状态可能循环，尝试恢复...`);
                // 游戏状态循环通常是由于某些玩家手牌数量异常导致的
                // 检查并修复玩家手牌
                if (!await (0, game_health_1.recoverGameState)(game)) {
                    await (0, game_health_1.handleHealthCheckFailure)(game, gameLoopInterval);
...
function updateCountdownMessage(message) {
    process.stdout.write(`\r${display_1.Style.YELLOW}${message}${display_1.Style.RESET}`);
}
/**
 * 清除所有倒计时消息
 */
function clearAllCountdownMessages() {
        ...
        try {
            ...
            if (currentPlayer.handTiles.length > 13) {
                try {
                    ...
                    (0, logger_1.warnLog)(`尝试使用备选方法处理AI玩家出牌...`);
                    display_manager_1.displayManager.printWarning(`尝试使用备选方法处理AI玩家出牌...`);
                    game.currentPlayerDiscard(tileIndex);
                    (0, logger_1.infoLog)(`已自动处理AI玩家回合`);
                    display_manager_1.displayManager.printSuccess(`已自动处理AI玩家回合`);
                }
                catch (fallbackError) {
                    (0, logger_1.errorLog)(`备选方法也失败: ${fallbackError instanceof Error ? fallbackError.message : String(fallbackError)}`);
                    display_manager_1.displayManager.printError(`备选方法也失败: ${fallbackError instanceof Error ? fallbackError.message : String(fallbackError)}`);
                    // 最后的尝试：直接使用玩家的discardTile方法
                    try {
                        const tileIndex = currentPlayer.handTiles.length - 1;
                        const fallbackDiscard = currentPlayer.discardTile(tileIndex);
                        if (fallbackDiscard) {
                            (0, logger_1.warnLog)(`使用备选方法出牌: ${fallbackDiscard.toString()}`);
                            display_manager_1.displayManager.printWarning(`使用备选方法出牌: ${fallbackDiscard.toString()}`);
                        }
                    }
                    catch (lastError) {
                        (0, logger_1.errorLog)(`所有出牌方法都失败，无法恢复状态: ${lastError instanceof Error ? lastError.message : String(lastError)}`);
                        display_manager_1.displayManager.printError(`所有出牌方法都失败，无法恢复状态: ${lastError instanceof Error ? lastError.message : String(lastError)}`);
                    }
                }
            }
            // 重置等待用户输入的状态

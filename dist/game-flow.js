...
exports.handleExcessHandTiles = handleExcessHandTiles;
exports.handlePlayerDraw = handlePlayerDraw;
exports.getAIDiscardIndex = getAIDiscardIndex;
...
class GameFlow {
    ...
    startGame() {
        ...
        if (this.players.length === 0) {
            display_manager_1.displayManager.printError("没有玩家，无法开始游戏");
            return;
        }
    ...
    dealInitialTiles() {
        ...
        for (const player of this.players) {
            if (player.handTiles.length !== 13) {
                display_manager_1.displayManager.printError(`严重错误: 玩家 ${player.name} 手牌数量不正确 (${player.handTiles.length}/13)，游戏无法继续`);
                return;
            }
            // 排序玩家手牌
            player.sortHand();
    ...
    currentPlayerDraw() {
        ...
        if (currentPlayer.handTiles.length !== 13) {
            (0, logger_1.debugLog)(`玩家 ${currentPlayer.name} 手牌数量不正确: ${currentPlayer.handTiles.length}`);
            return null;
        }
        const tile = this.tileManager.drawTile();
        if (tile) {
    ...
    currentPlayerDiscard(tileIndex) {
        ...
        if (currentPlayer.state !== player_1.PlayerState.ACTING) {
            ...
            if (currentPlayer.type === player_1.PlayerType.AI && currentPlayer.handTiles.length > 13) {
                display_manager_1.displayManager.printWarning(`AI玩家手牌超过13张，强制允许出牌以维持游戏状态正确性`);
 ...
 * @returns 是否已修复
 */
function handleExcessHandTiles(game, forcedFix = false) {
    // 检查每个玩家的手牌数量
    ...
    for (const player of allPlayers) {
        ...
        if (player.handTiles.length > 14) {
            needsFix = true;
            (0, logger_1.debugLog)(`检测到严重错误：玩家 ${player.name} 手牌数量(${player.handTiles.length})超过14张`);
            display_manager_1.displayManager.printError(`严重错误：玩家 ${player.name} 手牌数量(${player.handTiles.length})超过最大值14张，游戏无法继续`);
            // 不再执行修复逻辑，直接报错
            display_manager_1.displayManager.addToTurnLog(`游戏错误: 玩家 ${player.name} 手牌数量异常(${player.handTiles.length}/14)，游戏终止`);
            // 在实际应用中，这里可以添加退出程序的代码
            // 如 process.exit(1);
        }
        else if (player.handTiles.length < 13) {
            needsFix = true;
            (0, logger_1.debugLog)(`检测到严重错误：玩家 ${player.name} 手牌数量(${player.handTiles.length})少于13张`);
            display_manager_1.displayManager.printError(`严重错误：玩家 ${player.name} 手牌数量(${player.handTiles.length})少于最小值13张，游戏无法继续`);
            // 不再执行修复逻辑，直接报错
            display_manager_1.displayManager.addToTurnLog(`游戏错误: 玩家 ${player.name} 手牌数量异常(${player.handTiles.length}/13)，游戏终止`);
            // 在实际应用中，这里可以添加退出程序的代码
            // 如 process.exit(1);
        }
    }
 ...
 * @returns 是否成功摸牌
 */
function handlePlayerDraw(game) {
    // 获取当前玩家
    ...
    currentPlayer.state = player_1.PlayerState.ACTING; // 修正为合法的状态
    // 记录到回合日志
    display_manager_1.displayManager.addToTurnLog(`${currentPlayer.name} 摸了一张牌`);
    return true;
}
 ...
 * @returns 牌的索引
 */
function getAIDiscardIndex(player) {
    // 简单实现：返回最后一张牌的索引

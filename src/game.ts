import { Tile, TileType, sortTiles, createFullTileSet, shuffleTiles } from './tile';
import { Player, PlayerState, PlayerType } from './player';
import { PlayerAction, RuleEngine, GangType, HuType } from './rules';
...
export class Game {
  ...
  // 剩余牌数量
  public remainingTiles: number = 0;
  // 总牌数
  ...
  // 初始化游戏
  private initGame(players?: Player[]): void {
    console.log("初始化游戏...");
    ...
    if (CHECK_SEVEN_PAIRS) {
      ...
      if (aiPlayer) {
        const pairs = [
          ...
          new Tile(TileType.WAN, 4, 7), new Tile(TileType.WAN, 4, 8),
          new Tile(TileType.WAN, 5, 9), new Tile(TileType.WAN, 5, 10),
          new Tile(TileType.WAN, 6, 11), new Tile(TileType.WAN, 6, 12),
          // 直接完成最后一对
          new Tile(TileType.WAN, 7, 13), new Tile(TileType.WAN, 7, 14)
        ];
      ...
      if (aiPlayer) {
        const orphans = [
          ...
          new Tile(TileType.TIAO, 1, 3), new Tile(TileType.TIAO, 9, 4),
          new Tile(TileType.TONG, 1, 5), new Tile(TileType.TONG, 9, 6),
          new Tile(TileType.FENG, 1, 7), new Tile(TileType.FENG, 2, 8),
          new Tile(TileType.FENG, 3, 9), new Tile(TileType.FENG, 4, 10),
          new Tile(TileType.JIAN, 1, 11), new Tile(TileType.JIAN, 2, 12),
      ...
      if (aiPlayer) {
        const qingYiSe = [
          ...
          new Tile(TileType.WAN, 2, 4), new Tile(TileType.WAN, 3, 5), new Tile(TileType.WAN, 4, 6),
          new Tile(TileType.WAN, 5, 7), new Tile(TileType.WAN, 6, 8), new Tile(TileType.WAN, 7, 9),
          new Tile(TileType.WAN, 8, 10), new Tile(TileType.WAN, 9, 11), new Tile(TileType.WAN, 9, 12),
          // 最后一张将在游戏中摸到
          new Tile(TileType.WAN, 9, 13)
        ];
        aiPlayer.handTiles = qingYiSe;
        this.drawCount += 13; // 更新摸牌次数
        
        // 设置第14张牌为清一色的最后一张万牌
        const targetTile = new Tile(TileType.WAN, 2, 14);
      ...
      if (aiPlayer) {
        const pengPengHu = [
          new Tile(TileType.WAN, 1, 1), new Tile(TileType.WAN, 1, 2), new Tile(TileType.WAN, 1, 3),
          new Tile(TileType.TIAO, 2, 4), new Tile(TileType.TIAO, 2, 5), new Tile(TileType.TIAO, 2, 6),
        ...
        // 设置第14张牌为碰碰胡的最后一张对子
        const targetTile = new Tile(TileType.JIAN, 1, 14);
        this.tiles[0] = targetTile; // 设置为首张牌，保证下一次会摸到
      }
  ...
  // 从牌山抓牌
  private drawTileFromWall(isAfterGang: boolean = false): Tile | null {
  ...
  public currentPlayerDraw(): Tile | null {
    ...
    // 检查玩家是否可以自摸胡牌
    if (RuleEngine.canHu(currentPlayer, null, isTestMode)) {
    ...
    // 检查是否可以杠
    const gangResult = RuleEngine.canGang(currentPlayer);
    if (gangResult.canGang) {
      const gangTypeStr = gangResult.gangType === GangType.AN_GANG ? "暗杠" : "补杠";
  ...
  public currentPlayerDiscard(tileIndex: number): Tile | null {
    ...
    console.log(`玩家${player.name}状态: ${PlayerState[player.state]}`);
    console.log(`玩家手牌数量: ${player.handTiles.length}`);
    console.log(`玩家手牌: ${player.handTiles.map((t, i) => `${i}:${t.toString()}`).join(' ')}`);
    
    debugLog(`[game.currentPlayerDiscard] 玩家${player.name}状态=${PlayerState[player.state]}, 手牌数量=${player.handTiles.length}`);
    ...
    if (player.state !== PlayerState.ACTING) {
      ...
      if (player.type === PlayerType.AI && player.handTiles.length > 13) {
        ...
        debugLog(`[game.currentPlayerDiscard] AI玩家${player.name}手牌超过13张，强制设置状态: ${PlayerState[player.state]} -> 1`);
        player.state = PlayerState.ACTING;
      } else {
        debugLog(`[game.currentPlayerDiscard] 错误: 玩家${player.name}不处于行动状态(${PlayerState[player.state]})`);
        console.error(`错误: 玩家${player.name}不处于行动状态(${PlayerState[player.state]})`);
        return null;
      }
    }
    ...
    if (waitingPlayerIds.length > 0) {
      ...
      const waitingAIPlayers = waitingPlayerIds.filter(id => this.players[id].type === PlayerType.AI);
  ...
  // 检查其他玩家是否可以吃碰杠胡
  private checkOtherPlayersActions(tile: Tile): number[] {
    const actionPlayers: number[] = [];
    const allAllowedActions: PlayerAction[] = [];
    ...
    for (let i = 0; i < this.players.length; i++) {
      ...
      const player = this.players[i];
      let canAction = false;
      const playerAllowedActions: PlayerAction[] = [];
      
      // 检查胡
      if (RuleEngine.canHu(player, null, isTestMode)) {
        canAction = true;
        playerAllowedActions.push(PlayerAction.HU);
      }
      
      // 检查杠
      if (RuleEngine.canGang(player, tile).canGang) {
        canAction = true;
        playerAllowedActions.push(PlayerAction.GANG);
      }
      
      // 检查碰
      if (RuleEngine.canPeng(player, tile)) {
        canAction = true;
        playerAllowedActions.push(PlayerAction.PENG);
      }
      
      // 检查吃（只有下家可以吃）
      if ((i === (this.currentPlayerIndex + 1) % 4) && RuleEngine.canChi(player, tile)) {
        canAction = true;
        playerAllowedActions.push(PlayerAction.CHI);
      }
    ...
    if (actionPlayers.length > 0) {
      ...
      // 如果只有AI玩家可以操作，自动处理
      if (actionPlayers.every(id => this.players[id].type === PlayerType.AI)) {
        this.handleAIActions();
      } else {
        // 设置超时处理（实际实现中可能需要添加）
  ...
  private handleAIActions(): void {
    ...
    for (const playerId of waitingPlayerIds) {
      ...
      debugLog(`[handleAIActions] 处理玩家${playerId} (${player.name}), 类型=${PlayerType[player.type]}, 手牌数=${player.handTiles.length}`);
      ...
      // 胡
      if (this.pendingAction.allowedActions.includes(PlayerAction.HU) && RuleEngine.canHu(player, null, isTestMode)) {
      ...
      } else if (this.pendingAction.allowedActions.includes(PlayerAction.HU)) {
        debugLog(`[handleAIActions] 动作中包含胡牌，但检测到AI玩家${player.name}实际不能胡牌`);
      }
      
      // 杠
      if (!actionTaken && 
          this.pendingAction.allowedActions.includes(PlayerAction.GANG) && 
          RuleEngine.canGang(player, targetTile).canGang) {
      ...
      } else if (this.pendingAction.allowedActions.includes(PlayerAction.GANG)) {
        const gangResult = RuleEngine.canGang(player, targetTile);
        debugLog(`[handleAIActions] 动作中包含杠牌，但检测到AI玩家${player.name}实际${gangResult.canGang ? '可以' : '不能'}杠牌，杠牌类型=${gangResult.gangType}`);
      }
      
      // 碰
      if (!actionTaken && 
          this.pendingAction.allowedActions.includes(PlayerAction.PENG) && 
          RuleEngine.canPeng(player, targetTile)) {
      ...
      } else if (this.pendingAction.allowedActions.includes(PlayerAction.PENG)) {
        debugLog(`[handleAIActions] 动作中包含碰牌，但检测到AI玩家${player.name}实际${RuleEngine.canPeng(player, targetTile) ? '可以' : '不能'}碰牌`);
      }
      ...
      if (!actionTaken && 
          ...
          this.pendingAction.allowedActions.includes(PlayerAction.CHI)) {
        // 查找所有可能的吃牌组合
        const chiCombinations = RuleEngine.findChiCombinations(player.handTiles, targetTile);
      ...
      } else if (this.pendingAction.allowedActions.includes(PlayerAction.CHI)) {
    ...
    if (this.pendingAction.waitingPlayers.length > 0) {
      const remainingAIPlayers = this.pendingAction.waitingPlayers.filter(
        id => this.players[id].type === PlayerType.AI
      );
  ...
  // 玩家选择吃
  public playerChi(playerId: number, tiles: Tile[], targetTile: Tile): boolean {
  ...
  // 玩家选择碰
  public playerPeng(playerId: number, targetTile: Tile): boolean {
    if (this.state !== GameState.WAITING_ACTION || !this.pendingAction) {
      return false;
    }
  ...
  public playerGang(playerId: number, targetTile: Tile | null, isTestMode: boolean = false): boolean {
    ...
    if (isTestMode) {
      ...
      console.log(`杠后亮出牌组: ${player.revealedSets.map(set => 
        `${set.type}[${set.tiles.map(t => t.toString()).join(',')}]${set.source ? `(${set.source})` : ''}`
      ).join(', ')}`);
    }
    ...
    if (tile) {
      ...
      if (isTestMode) {
        ...
        let totalRevealedTiles = this.players.reduce((sum, p) => 
          sum + p.revealedSets.reduce((setSum, set) => setSum + set.tiles.length, 0), 0);
  ...
  // 获取游戏状态信息
  public getGameStateInfo(isTestMode = false, checkUpdates = false): string {
    ...
    // 计算各种杠牌调整
    const anGangCount = this.players.reduce((sum, player) => 
      sum + player.revealedSets.filter(set => set.type === 'GANG' && set.source === 'an').length, 0);
    
    const mingGangCount = this.players.reduce((sum, player) => 
      sum + player.revealedSets.filter(set => set.type === 'GANG' && set.source === 'ming').length, 0);
    
    const buGangCount = this.players.reduce((sum, player) => 
      sum + player.revealedSets.filter(set => set.type === 'GANG' && set.source === 'bu').length, 0);
    ...
    for (let player of this.players) {
      ...
      // 如果是测试模式，显示手牌详情
      if (isTestMode || player.type === PlayerType.HUMAN) {
  ...
  // 检查牌数平衡
  private checkCardBalance(isTestMode = false): string {
    // 计算手牌总数
  ...
  public getAvailableActions(): PlayerAction[] {
    if (this.state === GameState.WAITING_ACTION && this.pendingAction) {
      const humanWaitingIndex = this.pendingAction.waitingPlayers.find(
        id => this.players[id].type === PlayerType.HUMAN
      );
  ...
  // 在测试模式下自动为人类玩家打出一张牌
  public autoPlayForHumanInTestMode(): boolean {
    ...
    if (player.state !== PlayerState.ACTING) {
      console.log(`测试模式: 自动出牌失败，当前玩家状态不是ACTING: ${PlayerState[player.state]}`);
      return false;
    }
  ...
  private drawTileDirectly(): Tile | null {
    if (this.remainingTiles <= 0) {
      console.log("牌山已空，无法抓牌");
      this.state = GameState.ENDED;
      return null;
  ...
  // 在Game类中增加一个紧急修复方法
  private forceAIPlayerDiscard(): boolean {
    ...
    for (let i = 0; i < this.players.length; i++) {
      ...
      if (player.type === PlayerType.AI && player.handTiles.length > 13) {
        ...
        if (lastTile) {
          ...
          this.players[this.currentPlayerIndex].state = PlayerState.ACTING;
  ...
  public runGameCycle(): void {
    ...
    // 获取当前玩家
    const currentPlayer = this.players[this.currentPlayerIndex];
    debugLog(`[runGameCycle] 当前玩家: ${currentPlayer.name}, 类型=${PlayerType[currentPlayer.type]}, 状态=${PlayerState[currentPlayer.state]}, 手牌=${currentPlayer.handTiles.length}`);
    console.log(`当前玩家: ${currentPlayer.name}, 类型: ${PlayerType[currentPlayer.type]}, 状态: ${PlayerState[currentPlayer.state]}, 手牌: ${currentPlayer.handTiles.length}`);
  ...
  private handleAIAction(player: Player): void {
    ...
    // 确保AI玩家处于行动状态
    if (player.state !== PlayerState.ACTING) {
      debugLog(`[handleAIAction] 强制修正AI玩家${player.name}状态: ${PlayerState[player.state]} -> ACTING`);
      console.log(`强制修正AI玩家${player.name}状态: ${PlayerState[player.state]} -> ACTING`);
      player.state = PlayerState.ACTING;
    }
   ...
   * @param playerId 玩家ID
   */
  public playerPass(playerId: number): boolean {
  ...
  // 添加缺少的playerHu方法
  public playerHu(playerId: number, targetTile: Tile | null, huType: HuType = HuType.PING_HU): boolean {
    const player = this.players[playerId];
    if (!player) {
    ...
    if (huType === HuType.PING_HU) {
      // 重新分析牌型
      if (RuleEngine.isSevenPairs(finalHand)) {
        actualHuType = HuType.SEVEN_PAIRS;
        console.log(`检测到特殊牌型: 七对`);
      } else if (RuleEngine.isThirteenOrphans(finalHand)) {
        actualHuType = HuType.THIRTEEN_ORPHANS; 
        console.log(`检测到特殊牌型: 十三幺`);
      } else if (RuleEngine.isQingYiSe(finalHand, player.revealedSets)) {
        actualHuType = HuType.QING_YI_SE;
        console.log(`检测到特殊牌型: 清一色`);
      } else if (RuleEngine.isPengPengHu(finalHand, player.revealedSets)) {
        actualHuType = HuType.PENG_PENG_HU;
    ...
    console.log(`胡牌类型: ${HuType[actualHuType]}`);
    
    // 显示可视化牌型概要
    console.log(RuleEngine.getHandPatternVisualization(finalHand));
    
    // 输出详细的牌型分析
    if (DEBUG_MODE) {
      const handAnalysis = RuleEngine.analyzeHandTiles(finalHand);
      console.log(`--- 胡牌详细分析 ---`);
      ...
      if (handAnalysis.pairs.length > 0) {
        console.log(`找到的对子: ${handAnalysis.pairs.map(pair => 
          pair.map(t => t.toString()).join(',')
        ).join(' | ')}`);
      }
      if (handAnalysis.triples.length > 0) {
        console.log(`找到的刻子: ${handAnalysis.triples.map(triple => 
          triple.map(t => t.toString()).join(',')
        ).join(' | ')}`);
      }
    }
    ...
    const scoreInfo = RuleEngine.calculateScore(player);
    player.score += scoreInfo.score;
    debugLog(`[playerHu] 得分计算完成: ${scoreInfo.score}, 玩家总分: ${player.score}`);
    console.log(`得分: ${scoreInfo.score} (${HuType[scoreInfo.huType]})`);
    ...
    if (player.revealedSets.length > 0) {
      console.log(`已亮出牌组: ${player.revealedSets.map(set => 
  ...
  public nextTurn(): void {
    ...
    const currentPlayer = this.players[this.currentPlayerIndex];
    console.log(`当前玩家ID: ${this.currentPlayerIndex} (${currentPlayer.name}), 状态: ${PlayerState[currentPlayer.state]}`);
    debugLog(`[nextTurn] 当前玩家: ${currentPlayer.name}, 状态=${PlayerState[currentPlayer.state]}`);
    ...
    console.log(`摸牌前，下一玩家状态: ${PlayerState[nextPlayer.state]}`);
    console.log(`摸牌前，下一玩家手牌数: ${nextPlayer.handTiles.length}`);
    debugLog(`[nextTurn] 摸牌前状态检查: 游戏=${GameState[this.state]}, 玩家=${PlayerState[nextPlayer.state]}, 手牌=${nextPlayer.handTiles.length}`);
    ...
    if (nextPlayer.state !== PlayerState.ACTING) {
      ...
      debugLog(`[nextTurn] 修正当前玩家状态: ${PlayerState[nextPlayer.state]} -> ACTING`);
      nextPlayer.state = PlayerState.ACTING;
    }
    
    // 如果是AI玩家并且有足够的牌，立即执行出牌操作
    if (nextPlayer.type === PlayerType.AI && nextPlayer.handTiles.length > 13) {

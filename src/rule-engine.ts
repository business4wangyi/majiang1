...
import { WinConditions } from './win-conditions/index';
...
export class RuleEngine {
  ...
  static findChiCombinations(handTiles: Tile[], tile: Tile): Tile[][] {
    ...
    for (let i = -2; i <= 0; i++) {
      ...
      if (values.some(v => v < 1 || v > 9)) continue;
      ...
      if (tiles.every(t => t !== undefined)) {
        combinations.push(tiles as Tile[]);
      }
    }
  ...
  static canPeng(player: Player, tile: Tile): boolean {
    ...
    // 检查是否有两张相同的牌
    const sameTiles = player.handTiles.filter(t => 
   ...
   * @returns 明杠检查结果
   */
  static checkMingGang(player: Player, tile: Tile): { canGang: boolean, gangType: GangType | null, tiles?: Tile[] } {
    // 检查是否有三张相同的牌
    const sameTiles = player.handTiles.filter(t => 
      t.type === tile.type && t.value === tile.value
    );
   ...
   * @returns 暗杠检查结果
   */
  static checkAnGang(player: Player): { canGang: boolean, gangType: GangType | null, tiles?: Tile[] } {
    // 检查是否可以暗杠（手牌中有4张相同的牌）
    const tileGroups = new Map<string, {count: number, tiles: Tile[]}>();
   ...
   * @returns 补杠检查结果
   */
  static checkBuGang(player: Player, allPlayers: Player[] = []): { canGang: boolean, gangType: GangType | null, tiles?: Tile[] } {
    // 检查是否可以补杠（已经碰过的牌，再摸到第四张）
    for (const set of player.revealedSets) {
      if (set.type === 'PENG') {
        const firstTile = set.tiles[0];
        // 检查手牌中是否有相同的牌
        const matchingTile = player.handTiles.find(t => 
   ...
   * @returns 抢杠检查结果
   */
  static checkQiangGang(player: Player, tile: Tile, currentPlayer: Player): { canGang: boolean, gangType: GangType | null, tiles?: Tile[] } {
    // 检查当前玩家是否有一个碰的牌组，并且手中有对应的第四张牌
    const hasPengSet = currentPlayer.revealedSets.some(set => 
      set.type === 'PENG' && set.tiles[0].equals(tile)
    );
   ...
   * @returns 所有可能的杠牌组合
   */
  static findGangCombinations(player: Player, tile: Tile | null = null): {type: GangType, tiles: Tile[]}[] {
    const combinations: {type: GangType, tiles: Tile[]}[] = [];
  ...
  static canHu(
    ...
    const result = WinConditions.canHu(player, tile, gameState);
    return result.canHu;
  }
  ...
  static getHuDetails(
    ...
    return WinConditions.canHu(player, tile, gameState);
  }
   ...
   * @returns 可用操作列表
   */
  static getAvailableActions(player: Player, tile: Tile | null = null): PlayerAction[] {
   ...
   * @returns 是否为孤张牌
   */
  static isOrphan(tile: Tile): boolean {
    // 字牌
   ...
   * @returns 是否为边张牌
   */
  static isEdge(tile: Tile): boolean {
    // 只有数字牌才可能是边张
   ...
   * @returns 向听数
   */
  static calculateShanten(player: Player): number {
   ...
   * @returns 向听数
   */
  static calculateRegularShanten(handTiles: Tile[]): number {
    // 为了简化，使用较为简单的算法估算向听数
    ...
    Object.values(groups).forEach(group => {
      if (group.length >= 3) {
        sets++; // 刻子
        if (group.length >= 4) potentialSets += 0.5; // 四张牌可以额外形成杠或拆分
   ...
   * @returns 向听数
   */
  static calculateSevenPairsShanten(handTiles: Tile[]): number {
    // 按花色和点数分组
    const groups: { [key: string]: Tile[] } = {};
  ...
  static calculateThirteenOrphansShanten(handTiles: Tile[]): number {
    ...
    const hasTiao9 = handTiles.some(t => t.type === TileType.TIAO && t.value === 9);
    const hasTong1 = handTiles.some(t => t.type === TileType.TONG && t.value === 1);
    const hasTong9 = handTiles.some(t => t.type === TileType.TONG && t.value === 9);
    
    // 检查是否包含所有风牌
    const hasFeng1 = handTiles.some(t => t.type === TileType.FENG && t.value === 1);
    const hasFeng2 = handTiles.some(t => t.type === TileType.FENG && t.value === 2);
    const hasFeng3 = handTiles.some(t => t.type === TileType.FENG && t.value === 3);
    const hasFeng4 = handTiles.some(t => t.type === TileType.FENG && t.value === 4);
    
    // 检查是否包含所有箭牌
    const hasJian1 = handTiles.some(t => t.type === TileType.JIAN && t.value === 1);
    const hasJian2 = handTiles.some(t => t.type === TileType.JIAN && t.value === 2);
   ...
   */
  private static countSequences(sortedTiles: Tile[]): { complete: number; potential: number } {
    let complete = 0;
    let potential = 0;
    
    // 按花色分组
    const tilesByType: { [key: string]: Tile[] } = {};
    sortedTiles.forEach(tile => {
      // 只考虑数牌
      if (tile.type === TileType.WAN || tile.type === TileType.TIAO || tile.type === TileType.TONG) {
    ...
    Object.values(tilesByType).forEach(typeGroup => {
      ...
      typeGroup.forEach(tile => {
        if (!valueCount[tile.value]) valueCount[tile.value] = 0;
  ...
  static calculateScore(
    ...
    return ScoreCalculator.calculateScore(player, huType, gameState);
  }
   ...
   * @returns 牌型分析结果
   */
  static analyzeHand(player: Player): {
    tiles: Tile[],
    sets: {type: string, tiles: Tile[]}[],
    pairs: {tiles: Tile[]}[],
    remaining: Tile[],
    shanten: number,
    suggestions: {tile: Tile, reason: string}[]
  } {
    const handTiles = [...player.handTiles];
    ...
    // 找出刻子
    const sets: {type: string, tiles: Tile[]}[] = [];
    for (const [key, tiles] of tileGroups.entries()) {
      if (tiles.length >= 3) {
    ...
    // 提供出牌建议
    const suggestions: {tile: Tile, reason: string}[] = [];
    ...
    if (shanten === 0) {
      ...
      for (const type of Object.values(TileType)) {
        for (let value = 1; value <= 9; value++) {
          // 跳过不存在的牌（字牌只有1-7）
          if ((type === TileType.FENG && value > 4) || 
              (type === TileType.JIAN && value > 3)) {
            continue;
          }
          
          // 创建测试牌
          const testTile = new Tile(type as TileType, value, 0);
      ...
      if (goodTiles.length > 0) {
        ...
        if (neutralTiles.length > 0) {
          // 优先打出字牌和边张
          const edgeTiles = neutralTiles.filter(item => 
            this.isOrphan(item.tile) || this.isEdge(item.tile)
          );
   ...
   * @returns 是否相同
   */
  static isSameTile(tile1: Tile, tile2: Tile): boolean {

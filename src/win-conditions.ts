...
export class WinConditions {
  ...
  private static isValidSet(set: TileSet): boolean {
    ...
    switch (set.type) {
      case 'CHI': // 顺子
        ...
        if (!this.isNumberTile(tiles[0])) return false;
   ...
   * 判断牌组是否为标准胡牌型（4组+1对）
   */
  static isStandardHu(tiles: Tile[], revealedSets: TileSet[] = []): boolean {
    // 计算总牌数，考虑杠牌的情况
    ...
    if (revealedSetCount > 4) return false;
   ...
   * 递归检查是否能组成指定数量的组合+1对
   */
  static canFormSetsRecursive(tiles: Tile[], depth = 0, combineHistory: string[] = [], requiredSets = 4): boolean {
    // 检查递归深度，防止无限递归
   ...
   */
  static canFormTriples(tiles: Tile[], depth: number, combineHistory: string[], requiredSets: number): boolean {
    // 如果没有牌了，说明已经成功组合
    ...
    if (requiredSets === 0 && tiles.length > 0) return false;
    
    // 如果剩余牌的数量不足以组成需要的组合数量，返回false
    if (tiles.length < requiredSets * 3) return false;

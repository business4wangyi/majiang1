...
// 全局变量区域
// 用于记录当前回合的操作日志
let currentTurnLog: string[] = [];
let currentDrawPlayerIndex: number = -1;
// 测试模式相关配置
...
// 添加操作到当前回合日志
function addToTurnLog(message: string): void {
  // 检查消息是否为空
...
// 显示AI操作信息
function displayAIAction(player: Player, action: string): void {
...
// 显示AI思考过程
function displayAIThinking(player: Player, handDescription: string): void {
  // 只在测试模式且有手牌描述时显示
...
// 显示重要事件通知
function displayImportantEvent(message: string): void {
...
// 设置自动打牌索引，确保按顺序打出手牌
function getNextAutoDiscardIndex(handTilesLength: number): number {
  // 如果是第一次打牌，从第1张开始（索引0）
...
// 获取玩家选择的吃牌组合
async function getPlayerChiChoice(combinations: Tile[][]): Promise<number> {
  try {
    // 测试模式下，自动选择第一个组合
    if (isTestMode) {
...
// 游戏主循环
async function gameLoop(game: Game): Promise<void> {
  // 设置循环计数器和状态跟踪
  let turnCounter = 0;
  let stuckCounter = 0;
    ...
    try {
      ...
      if (currentStateHash === lastStateHash) {
        ...
        if (stuckCounter >= MAX_STUCK_TURNS) {
          ...
          debugLog(`当前玩家: ${currentPlayer.name}, 类型: ${PlayerType[currentPlayer.type]}, 状态: ${PlayerState[currentPlayer.state]}`);
          ...
          if (currentPlayer.type === PlayerType.AI) {
            ...
            if (game.state !== GameState.PLAYING) {
              debugLog(`强制修正游戏状态: ${GameState[game.state]} -> PLAYING`);
              game.state = GameState.PLAYING;
            }
            
            // 强制设置玩家状态为ACTING
            if (currentPlayer.state !== PlayerState.ACTING) {
              debugLog(`强制修正玩家状态: ${PlayerState[currentPlayer.state]} -> ACTING`);
      ...
      else if (game.state === GameState.WAITING_ACTION) {
  ...
  debugLog(`游戏循环结束: 状态=${GameState[game.state]}, 总轮次=${turnCounter}, 干预次数=${forcedInterventionCounter}`);
}

// 生成游戏状态哈希，用于检测循环
function generateGameStateHash(game: Game): string {
...
// 分析AI手牌中的可能组合
function analyzeAIHand(player: Player): string[] {
  const result: string[] = [];
  ...
  for (const type of [TileType.WAN, TileType.TIAO, TileType.TONG]) {
    ...
    for (let startValue = 1; startValue <= 7; startValue++) {
      ...
      // 如果只有部分顺子，也显示
        ...
        result.push(`${Style.DIM}${Style.GREEN}部分顺子[${type}:${
          (hasStartValue ? startValue : '') +
          (hasMiddleValue ? (hasStartValue ? ',' : '') + (startValue + 1) : '') +
          (hasEndValue ? (hasStartValue || hasMiddleValue ? ',' : '') + (startValue + 2) : '')
        }]${Style.RESET}`);
  ...
  // 检查听牌状态
  const tingCheckResult = RuleEngine.canTing(player);
...
// 按牌型和点数对牌进行分组
function groupTilesByTypeAndValue(tiles: Tile[]): Map<string, Tile[]> {
...
// 处理玩家动作
async function handlePlayerAction(game: Game): Promise<void> {
  // 如果没有待处理的操作，则结束
  ...
  try {
    ...
    for (const waitingPlayerId of game.pendingAction.waitingPlayers) {
      ...
      else if (waitingPlayer.type === PlayerType.AI) {
        // AI玩家的处理逻辑
...
// 保存游戏日志到文件
function saveGameLogToFile(game: Game, errorMessage: string): void {
  try {
    const logDir = path.join(__dirname, '../logs');
    ...
    game.players.forEach((player, index) => {
      logContent.push(`\n--- 玩家 ${index}: ${player.name} (${PlayerType[player.type]}) ---`);
      logContent.push(`状态: ${PlayerState[player.state]}`);
      ...
      logContent.push(`已亮出牌组: ${player.revealedSets.map(set => 
        `${set.type}[${set.tiles.map(t => t.toString()).join(',')}]`).join(' ')}`);
    });
    
    // 如果有待处理的动作，也记录下来
    if (game.state === GameState.WAITING_ACTION) {
...
// 添加一个新的辅助函数来处理AI出牌
async function handleAIDiscard(game: Game, player: Player): Promise<boolean> {
  console.log(`执行AI玩家 ${player.name} 的出牌操作`);
  debugLog(`=== AI出牌处理开始 ===`);
  
  // 确保是AI玩家且有足够的牌
  if (player.type !== PlayerType.AI || player.handTiles.length <= 13) {
    console.log(`不需要处理出牌：玩家类型=${PlayerType[player.type]}，手牌数量=${player.handTiles.length}`);
    debugLog(`跳过处理: 玩家类型=${PlayerType[player.type]}或手牌数量不足(${player.handTiles.length})`);
    return false;
  }
  ...
  if (player.state !== PlayerState.ACTING) {
    console.log(`AI玩家状态不对，当前状态: ${PlayerState[player.state]}，强制设置为ACTING`);
    debugLog(`修正玩家状态: ${PlayerState[player.state]} -> ${PlayerState.ACTING}`);
    player.state = PlayerState.ACTING;
  }
  
  // 检查游戏状态
  if (game.state !== GameState.PLAYING) {
    console.log(`游戏状态不是PLAYING，当前状态: ${GameState[game.state]}，强制设置为PLAYING`);
    debugLog(`修正游戏状态: ${GameState[game.state]} -> ${GameState.PLAYING}`);
    game.state = GameState.PLAYING;
  }
  ...
  try {
    ...
    console.log(`所有出牌尝试都失败，检查游戏状态: ${GameState[game.state]}, 玩家状态: ${PlayerState[player.state]}`);
    debugLog(`所有常规出牌策略失败`);
...
async function main(): Promise<void> {
  ...
  if (process.argv.includes('--test-special-patterns')) {
    console.log("\n测试大四喜牌型:");
    const bigFourWindsResult = RuleEngine.testBigFourWinds();
    console.log(`大四喜测试结果: ${bigFourWindsResult ? '通过' : '失败'}`);
    
    console.log("\n测试小四喜牌型:");
    const smallFourWindsResult = RuleEngine.testSmallFourWinds();
    console.log(`小四喜测试结果: ${smallFourWindsResult ? '通过' : '失败'}`);
    
    console.log("\n测试大三元牌型:");
    const bigThreeDragonsResult = RuleEngine.testBigThreeDragons();
    console.log(`大三元测试结果: ${bigThreeDragonsResult ? '通过' : '失败'}`);
    
    console.log("\n测试小三元牌型:");
    const smallThreeDragonsResult = RuleEngine.testSmallThreeDragons();
    console.log(`小三元测试结果: ${smallThreeDragonsResult ? '通过' : '失败'}`);
    
    console.log("\n测试字一色牌型:");
    const allHonorsResult = RuleEngine.testAllHonors();
  ...
  if (isTestMode) {
    ...
    RuleEngine.testThirteenOrphans();
    
    // 测试七对逻辑
    console.log("\n测试七对逻辑:");
    RuleEngine.testSevenPairs();

    // 测试风牌牌型
    console.log("\n测试风牌处理逻辑:");
    RuleEngine.testWindTiles();
    
    // 测试箭牌牌型
    console.log("\n测试箭牌处理逻辑:");
    RuleEngine.testArrowTiles();
    
    // 测试所有特殊牌型胡牌功能
    console.log("\n测试所有特殊牌型胡牌功能:");
    RuleEngine.testAllSpecialHuFunctions();
  }
  ...
  if (CHECK_SEVEN_PAIRS || CHECK_THIRTEEN_ORPHANS || CHECK_QING_YI_SE || CHECK_PENG_PENG_HU || CHECK_STANDARD_HU) {
    ...
    if (CHECK_SEVEN_PAIRS) {
      console.log("\n=== 测试七对牌型 ===");
      const sevenPairsResult = RuleEngine.testSevenPairs();
      console.log(`七对牌型测试结果: ${sevenPairsResult ? '✓ 通过' : '✗ 失败'}`);
    }
    
    if (CHECK_THIRTEEN_ORPHANS) {
      console.log("\n=== 测试十三幺牌型 ===");
      const thirteenOrphansResult = RuleEngine.testThirteenOrphans();
      console.log(`十三幺牌型测试结果: ${thirteenOrphansResult ? '✓ 通过' : '✗ 失败'}`);
    }
    
    if (CHECK_QING_YI_SE) {
      console.log("\n=== 测试清一色牌型 ===");
      const qingYiSeResult = RuleEngine.testQingYiSe();
      console.log(`清一色牌型测试结果: ${qingYiSeResult ? '✓ 通过' : '✗ 失败'}`);
    }
    
    if (CHECK_PENG_PENG_HU) {
      console.log("\n=== 测试碰碰胡牌型 ===");
      const pengPengHuResult = RuleEngine.testPengPengHu();
      console.log(`碰碰胡牌型测试结果: ${pengPengHuResult ? '✓ 通过' : '✗ 失败'}`);
    }
    
    if (CHECK_STANDARD_HU) {
      console.log("\n=== 测试标准和牌牌型 ===");
      const standardHuResult = RuleEngine.testStandardHu();
  ...
  if (CHECK_BIG_FOUR_WINDS) {
    console.log("\n🎮 麻将游戏 - 特殊牌型测试 - 大四喜");
    const result = RuleEngine.testBigFourWinds();
  ...
  if (CHECK_BIG_THREE_DRAGONS) {
    console.log("\n🎮 麻将游戏 - 特殊牌型测试 - 大三元");
    const result = RuleEngine.testBigThreeDragons();
  ...
  if (CHECK_SMALL_FOUR_WINDS) {
    console.log("\n🎮 麻将游戏 - 特殊牌型测试 - 小四喜");
    const result = RuleEngine.testSmallFourWinds();
  ...
  if (CHECK_SMALL_THREE_DRAGONS) {
    console.log("\n🎮 麻将游戏 - 特殊牌型测试 - 小三元");
    const result = RuleEngine.testSmallThreeDragons();
  ...
  if (CHECK_ALL_HONORS) {
    console.log("\n🎮 麻将游戏 - 特殊牌型测试 - 字一色");
    const result = RuleEngine.testAllHonors();
  ...
  const gameLoopTimer = setInterval(() => {
    // 如果游戏处于运行状态或等待动作状态
    if (game.state === GameState.PLAYING || game.state === GameState.WAITING_ACTION) {
      // 运行游戏循环，处理AI玩家行动
      game.runGameCycle();
      ...
      if (isTestMode) {
        ...
        if (game.state === GameState.PLAYING && 
          ...
          if (currentPlayer.lastDrawnTile) {
            // 找到最后摸到的牌的索引
            cardIndex = currentPlayer.handTiles.findIndex(t => 
              t.id === currentPlayer.lastDrawnTile?.id);
          }
